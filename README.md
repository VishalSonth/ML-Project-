...
1. **Data Ingestion**: Reading data from local or remote databases and splitting into train/test sets.
2. **Data Transformation**: Handling missing values, categorical encoding, and feature scaling using Scikit-learn Pipelines.
3. **Model Trainer**: Testing multiple regression algorithms (Linear Regression, Random Forest, etc.) and selecting the best performer based on R2 Score.
...
☁️ Deployment on AWS
The application is hosted on AWS Elastic Beanstalk. The deployment is automated via GitHub Actions and AWS CodePipeline. Any change pushed to the main branch triggers a new build and deployment automatically.
